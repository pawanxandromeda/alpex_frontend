...
function Sidebar({ isCollapsed, setIsCollapsed }) {
  ...
  const hasAccess = (userDepartment, itemDepartments) => {
    if (!userDepartment || !itemDepartments) return false
    ...
    // Check if user's department is in the item's allowed departments array
    if (Array.isArray(itemDepartments)) {
      return itemDepartments.some(allowedDept => 
        allowedDept.toLowerCase() === dept
      )
    }
    
    // If itemDepartments is not an array (shouldn't happen with your structure)
    return itemDepartments.toLowerCase() === dept
  }
  ...
  useEffect(() => {
    ...
    const filterItems = () => {
      ...
      if (userRole === 'admin') {
        const adminItems = sidebarItems.filter(item => !item.department || item.department.length > 0);
        console.log('Admin filtered items:', adminItems);
        setFilteredSidebarItems(adminItems);
        return;
      }

      // Filter sidebar items based on department
          ...
          <ul className="space-y-1.5">
            {filteredSidebarItems.map((item) => (
              <li key={item.id || item.label}>
                ...
                {item.isDropdown && openDropdowns[item.id] && !isCollapsed && item.items && item.items.length > 0 && (
              ...
              <ul className="space-y-2">
                {filteredSidebarItems.map((item) => (
                  <li key={item.id || item.label}>
                    ...
                    {item.isDropdown && openDropdowns[item.id] && item.items && item.items.length > 0 && (
